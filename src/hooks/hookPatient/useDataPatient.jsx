...
export const useDataPatient = () => {
  const { patient: patientData } = storePatient()

  const [patient, setPatient] = useState([])
  const [patientExport, setPatientExport] = useState([])

  useEffect(() => {
    const result = patientData?.map(item => {
      ...
      return {
        ...
        hpeDetailId: item?.hpe_detail?.[0]?.hpe_detail_id,
        nroPolicy: item?.hpe_detail?.[0]?.nro_policy,
        hpeId: item?.hpe_detail?.[0]?.hpe_id,
        hpeName: item?.hpe_detail?.[0]?.hpe_name,
        contactName: item?.hpe_detail?.[0]?.contact_name
      }
    })
    setPatient(result)
